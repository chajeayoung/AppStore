...
public class CartDBBean {
    ...
    public void insertCart(CartDataBean cart) throws Exception {
        ...
        try {
            conn = getConnection();
            sql = "insert into cart (prd_id, buyer," +
            		"prd_name,buy_price,buy_count,prd_image) " +
            		"values (?,?,?,?,?,?)";
            pstmt = conn.prepareStatement(sql);
            
            pstmt.setInt(1, cart.getPrd_id());
            pstmt.setString(2, cart.getBuyer());
            pstmt.setString(3, cart.getPrd_name());
            pstmt.setInt(4, cart.getBuy_price());
            pstmt.setByte(5, cart.getBuy_count());
            pstmt.setString(6, cart.getPrd_image());
     ...
     public List<CartDataBean> getCart(String id, int count) throws Exception {
         ...
         try {
             ...
             while (rs.next()) {
            	 ...
            	 cart.setPrd_id(rs.getInt("prd_id"));
            	 cart.setPrd_name(rs.getString("prd_name"));
            	 cart.setBuy_price(rs.getInt("buy_price"));
            	 cart.setBuy_count(rs.getByte("buy_count")); 
            	 cart.setPrd_image(rs.getString("prd_image"));
